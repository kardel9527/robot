...
local function _regedit_table_enum( prefix , enum, enumtable )
	local prefix_table = enumtable[ prefix ]
	if not prefix_table then
		prefix_table = {}
		enumtable[prefix] = prefix_table
	end
	
	local enum_table = prefix_table[enum.name]
	if not enum_table then
		enum_table = {}
		prefix_table[enum.name] = enum_table
	end
	
	for _,v in ipairs(enum.value) do
		enum_table[v.name] = v.number
	end
end


function M.register( buffer, enumtable )
	c._env_register(P, buffer)

	local t = M.decode("google.protobuf.FileDescriptorSet", buffer)
	local packageName = nil
	for _,file in ipairs(t.file) do
		packageName = file.package
		for _,enumData in ipairs(file.enum_type) do
			_regedit_table_enum( packageName , enumData, enumtable )
		end
	end	
end

function M.register_file(filename, enumtable)
