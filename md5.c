...
static void bytes_to_hexstring (unsigned char *buf, int len)
{
	int i, j;
	char temp[32];
...
{
	return md5_makedata(src, (int)strlen(src), buf, len);
}
...
char *md5_makedata (const char *src, int srclen, char *buf, int len)
{
	...
	if (!src || srclen < 0 || !buf || len < 33)
		return NULL;
	MD5Init(&md5c);
	MD5Update(&md5c, (unsigned char *)src, srclen);
	MD5Final((unsigned char *)buf, &md5c);

	bytes_to_hexstring((unsigned char*)buf, 16);
